...
export class kindie1673022225443 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`ALTER TABLE "activity_log" ADD "kindergardenId" integer`
		);
		await queryRunner.query(
			`ALTER TABLE "activity_log" ADD "groupId" integer`
		);
		await queryRunner.query(
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "activity_log" DROP COLUMN "groupId"`
		);
		await queryRunner.query(
			`ALTER TABLE "activity_log" DROP COLUMN "kindergardenId"`
		);
	}
}
